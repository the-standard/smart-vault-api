...
const getDBData = async userAddress => {
  ...
  try {
    result = (await client.query('SELECT snapshot_at, tst, tst_value, euros, euros_value, rewards FROM user_pool_snapshots_sepolia where user_address = $1', [userAddress])).rows;
  } finally {
    client.release();
  }
  return result;
